...
def _convert_to_input_api(
    ...
    for obj in objects_map.values():
        ...
        input_objects.append(InputModel(name=obj_name, fields=fields))

    # Build tag lookup
    tags_by_id = {tag.id: tag for tag in api.tags}

    # Convert endpoints to InputView
    input_views: list[InputView] = []
    for endpoint in api.endpoints:
        # Build view name from method and path
        view_name = _build_view_name(endpoint.method, endpoint.path)
    ...
    return InputAPI(
        ...
        version=api.version,
        author="Median Code",
        description=api.description or f"API Generated by Median Code",
...
def _build_view_name(method: str, path: str) -> str:
    """Build a view name from HTTP method and path.

    :param method: HTTP method (GET, POST, etc.).
    :param path: URL path.
    :returns: PascalCase view name.
    """
