...
class DatabaseStack(Stack):
    def __init__(
        ...
        **kwargs,
        ...
        # Allow any resource within the VPC to reach RDS on 5432.
        # Using VPC CIDR avoids a circular cross-stack SG reference:
        # compute_stack → db_stack (DATABASE_URL) and db_stack → compute_stack
        # (ECS task SG) would create a cycle. VPC-CIDR ingress breaks the cycle
        # while still being safe — the VPC itself provides network isolation.
        self.security_group.add_ingress_rule(
            peer=ec2.Peer.ipv4(vpc.vpc_cidr_block),
            connection=ec2.Port.tcp(5432),
            description="Allow VPC resources to connect to RDS",
        )
